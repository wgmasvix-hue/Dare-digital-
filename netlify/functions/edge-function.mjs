...
function convertGeminiContentsToDeepSeek(contents, systemInstruction) {
  const messages = [];
  ...
  if (Array.isArray(contents)) {
    for (const item of contents) {
      ...
      if (typeof item.parts === 'string') {
        ...
        content = item.parts.map(p => (typeof p === 'string' ? p : p?.text || '')).join('\n');
      } else if (item.text) {
        content = item.text;
      } else {
...
async function callDeepSeek(messages, options = {}) {
  ...
  if (!res.ok) throw new Error(`DeepSeek error: ${await res.text()} (${res.status})`);
...
export const handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    Object.assign(body, event.queryStringParameters || {});
  }
  ...
  try {
    ...
    if (functionName === 'external-proxy') {
      ...
      return ok({ status: response.status, data: parsed || text });
    }
    ...
    if (['dara-ai', 'ai-chat', 'gemini'].includes(functionName)) {
      ...
      const pillars = `DARA operates under Zimbabwe's Education 5.0 framework: Teaching, Research, Community Service, Innovation, Industrialization.`;
      const baseSystem = `You are DARA (Digital Academic Research Assistant), an AI tutor for Zimbabwe and the African region. ${pillars} Always provide helpful, culturally relevant, and accurate responses.`;
    ...
    if (functionName === 'search-books') {
      ...
      const systemInstruction = `You are DARA. Extract keywords, faculty, and level from the user's request. Return JSON: { "keywords": [], "faculty": "All", "level": "All" }. Faculties: STEM, Agriculture, Health, Business, Education, Engineering, Law, Humanities, AI & Future Tech. Levels: Certificate, Diploma, HND, Degree, Masters, PhD.`;
    ...
    if (functionName === 'repository-sync') {
      ...
      return ok({ success: true, synced_count: syncedCount, message: `Successfully harvested ${syncedCount} new records from ${institution}.` });
    }

    return ok({ success: true, message: `Handled: ${functionName}` });
  } catch (error) {
