...
import {
  ...
  FlaskConical, Zap, Database, Globe, ClipboardCheck, MoreHorizontal
} from 'lucide-react';
...
const PRIMARY_LINKS = [
  ...
  { to: '/open-books', label: '1M+ Books', Icon: Globe,       cls: 'text-teal-500' },
  { to: '/tutor',      label: 'AI Tutor',  Icon: Sparkles,    cls: 'text-amber-500' },
  { to: '/research',   label: 'Research',  Icon: FlaskConical },
];

const MORE_LINKS = [
  { to: '/dspace-explorer', label: 'Repository',     Icon: Database,       cls: 'text-slate-400' },
  { to: '/advanced-search', label: 'Research DB',    Icon: Search,         special: true },
  { to: '/teacher-tools',   label: 'Lesson Planner', Icon: ClipboardCheck, cls: 'text-emerald-500' },
];
...
export default function NavBar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          ...
          <div className="relative" ref={moreRef}>
            ...
            <AnimatePresence>
              {isMoreOpen && (
                ...
                >
                  {MORE_LINKS.map(({ to, label, Icon, cls, special }) => (
                    <Link key={to} to={to} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors ${special ? 'text-indigo-600 hover:bg-indigo-50' : 'text-slate-700 hover:bg-slate-50'}`}>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          {user ? (
            <div className="relative" ref={dropdownRef}>
              <button onClick={() => setIsProfileOpen(v => !v)} aria-expanded={isProfileOpen} aria-haspopup="menu"
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div role="menu" initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.96 }} transition={{ duration: 0.12 }}
                    ...
                    {[
                      ...
                      <Link key={item.to} to={item.to} role="menuitem"
                        className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-colors ${isActive(item.to) ? 'bg-slate-100 font-black text-slate-900' : 'font-medium text-slate-700 hover:bg-slate-50'}`}>
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <Link to="/signup" className="px-4 py-2 rounded-full font-bold text-sm bg-slate-900 text-white hover:bg-slate-800 transition-all active:scale-95 shadow-sm">Join DARE</Link>
            </div>
          )}

          <button className="md:hidden p-2 rounded-xl text-slate-600 hover:bg-slate-100 transition-colors focus:outline-none"
            onClick={() => setIsMobileMenuOpen(v => !v)} aria-label={isMobileMenuOpen ? 'Close menu' : 'Open menu'} aria-expanded={isMobileMenuOpen}>
            <AnimatePresence mode="wait" initial={false}>
              <motion.div key={isMobileMenuOpen ? 'x' : 'menu'} initial={{ rotate: -90, opacity: 0, scale: 0.8 }} animate={{ rotate: 0, opacity: 1, scale: 1 }} exit={{ rotate: 90, opacity: 0, scale: 0.8 }} transition={{ duration: 0.12 }}>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2, ease: 'easeInOut' }}
            className="md:hidden bg-white border-b border-slate-200 overflow-hidden shadow-xl">
            <div className="px-5 py-5 flex flex-col gap-1">
              <div className="mb-3 border border-slate-200 rounded-xl p-2 bg-slate-50"><GlobalSearch /></div>
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-2 mb-1">Navigate</p>
              {ALL_MOBILE_LINKS.map(({ to, label, Icon, cls }) => (
                <Link key={to} to={to} aria-current={isActive(to) ? 'page' : undefined}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${isActive(to) ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100'}`}>
                  <Icon size={16} className={isActive(to) ? 'text-white' : (cls || 'text-slate-500')} />{label}
                </Link>
              ))}
              ...
              {!user ? (
                <div className="flex flex-col gap-2">
                  ...
                  <Link to="/signup" className="py-3 rounded-xl font-bold text-center bg-slate-900 text-white text-sm shadow">Join DARE</Link>
                </div>
              ) : (
                <>
                  ...
                  {[
                    ...
                    <Link key={item.to} to={item.to}
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${isActive(item.to) ? 'bg-slate-100 text-slate-900' : 'text-slate-700 hover:bg-slate-50'}`}>
                      <item.Icon size={16} className="text-slate-400" />{item.label}
                    </Link>
                  ))}
                  <button onClick={signOut} className="flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm text-red-600 hover:bg-red-50 transition-colors text-left w-full">
